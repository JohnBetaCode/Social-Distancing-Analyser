...
class calibrator():
    ...
    def __init__(self, img, img_scl=1.0, intrinsic=None, extrinsic=None):
        ...
        # Image source
        self.img_src = cv2.resize(src=img, 
        ...
        if self._intrinsic is not None:
            ...
            if (self.img_src.shape[1] != self._intrinsic["image_width"] and
                self.img_src.shape[0] != self._intrinsic["image_height"]):
                self.img_src = cv2.resize(self.img_src, 
                    (self._intrinsic["image_width"], 
                    self._intrinsic["image_height"]),
                    interpolation=int(cv2.INTER_AREA))

            # Undistord image if intrinsic calibration
            self.img_src = cv2.remap(self.img_src, 
                self._intrinsic["map1"], self._intrinsic["map2"],
                cv2.INTER_LINEAR) 
        ...
        self._win_bird_max_width = 500.
        self._win_bird_max_height = 500.
    ...
    def _draw_measure(self, img):
        ...
        if len(self.bird_pts) == 1:
            cv2.line(img=img, pt1=self.bird_pts[0], 
                pt2=tuple(self.bird_mouse_current_pos), 
                color=(255, 0, 255), thickness=2)

        elif len(self.bird_pts) == 2:
            cv2.line(img=img, pt1=self.bird_pts[0], pt2=self.bird_pts[1], 
    ...
    def _draw_points(self, img):
        ...
        if self._Mpts["p1"] is not None and self._Mpts["p3"] is not None:
            dotline(src=img, p1=self._Mpts["p1"], p2=self._Mpts["p3"], 
                color=(255, 255, 255), thickness=1, Dl=5)
        if self._Mpts["p2"] is not None and self._Mpts["p4"] is not None:
            dotline(src=img, p1=self._Mpts["p2"], p2=self._Mpts["p4"], 
                color=(255, 255, 255), thickness=1, Dl=5)
    ...
    def _draw_polygon(self, img):
        ...
        if len(self.pts) == 2:
            cv2.line(img=img, pt1=self.pts[0], pt2=self.pts[1], 
                color=(255, 0, 0), thickness=2)
        ...
        # Draw a polygon if more than 2 points
            ...
            cv2.polylines(img=img, pts=[pts], isClosed=True, 
                color=(255, 0, 0), thickness=2)
    ...
    def _draw_rulers(self, img, pose):
        ...
        dotline(
            src=img, p1=(0, pose[1]), p2=(img.shape[1], pose[1]), 
            color=(0, 0, 255), thickness=1, Dl=5)
        # Draw vertical line
        dotline(
            src=img, p1=(pose[0], 0), p2=(pose[0], img.shape[0]), 
            color=(0, 0, 255), thickness=1, Dl=5)
    ...
    def _draw_skyview(self):
        ...
        if self._M is not None:
            ...
            self.img_sky = cv2.warpPerspective(
                dsize=(self.warp_width, self.warp_height),
                src=self.img_src, 
                M=self._M) 
            ...
            for i in range(0, self.warp_width, self._WIN_WARP_VER_DIV):
                cv2.line(img=self.img_sky, pt1=(i , 0), pt2=(i, self.warp_height), 
                    color=(0, 255, 0), thickness=1)
            for i in range(0, self.warp_height, self._WIN_WARP_HOZ_DIV):
                cv2.line(img=self.img_sky, pt1=(0 , i), pt2=(self.warp_width, i), 
                    color=(0, 255, 0), thickness=1)

            pts = np.array((self._Mdst_pts["p1"], self._Mdst_pts["p2"], 
                self._Mdst_pts["p3"], self._Mdst_pts["p4"]), np.int32)
            cv2.polylines(img=self.img_sky, pts=[pts], isClosed=True, 
                color=(255, 0, 0), thickness=2)
            cv2.polylines(img=self.img_sky, pts=[np.array(self._MOriginpts)], 
                isClosed=True, color=(255, 255, 0), thickness=2)

            # Draw surface lines
            dotline(src=self.img_sky, 
                p1=self._Mdst_pts["p1"], p2=self._Mdst_pts["p3"], 
                color=(255, 255, 255), thickness=1, Dl=5)
            dotline(src=self.img_sky, 
                p1=self._Mdst_pts["p2"], p2=self._Mdst_pts["p4"], 
                color=(255, 255, 255), thickness=1, Dl=5)

            # Draw matrix surfarce projection points
            for key, pt in self._Mdst_pts.items():
                if pt is not None:
                    cv2.circle(img=self.img_sky, center=pt, radius=3, 
                        color=(0, 0, 255), thickness=-1)
                    cv2.putText(img=self.img_sky, text='{}'.format(key), 
                        org=(pt[0] - 8, pt[1] - 8), 
                        fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.7, 
                        color=(0, 0, 0), thickness=3, lineType=cv2.LINE_AA) 
                    cv2.putText(img=self.img_sky, text='{}'.format(key), 
            ...
            self._draw_rulers(img=self.img_sky, 
                pose=self.bird_mouse_current_pos)
        ...
        if vfscl > hfscl:
            self.img_sky=image_resize(image=self.img_sky, 
                width=None, height=int(self._win_bird_max_height), 
                inter=cv2.INTER_AREA)
        else:
            self.img_sky=image_resize(image=self.img_sky, 
                width=int(self._win_bird_max_width), height=None, 
                inter=cv2.INTER_AREA)
    ...
    def _update_pix_relation(self, dm, dp, relation):
        ...
        if relation == "dv":
            ...
            printlog(msg="Vertical relation updated, new values are "
                "ppmx={:.2f}[pix/m], ppmy={:.2f}[pix/m]".format(
                    self.ppmx, self.ppmy), 
                msg_type="USER")
        elif relation == "dh":
            self.ppmx = float(dm/dp)
            printlog(msg="Horizontal relation updated, new values are "
                "ppmx={:.2f}[pix/m], ppmy={:.2f}[pix/m]".format(
                    self.ppmx if self.ppmx is not None else 0.0, 
                    self.ppmy if self.ppmy is not None else 0.0), 
                msg_type="USER")
        else:
            printlog(msg="No valid relation to update", 
                msg_type="ERROR")
    ...
    def _calibrate_extrinsic(self):
        ...
        for pt in self._Mpts.values():
            if pt is None:
                printlog(msg="No transformation matrix yet".format(key), 
                    msg_type="WARN")
                return
        ...
        dst_pt_ltop = get_projection_point_dst(
            coords_src=(0, 0), M=M)
        dst_pt_rtop = get_projection_point_dst(
            coords_src=(self.img_width, 0), M=M)
        dst_pt_lbottom = get_projection_point_dst(
            coords_src=(0, self.img_height), M=M)
        dst_pt_rbottom = get_projection_point_dst(
            coords_src=(self.img_width, self.img_height), M=M)
        self._MOriginpts = [dst_pt_ltop, dst_pt_rtop, 
            dst_pt_rbottom, dst_pt_lbottom]

        if not self._in_gerion:
            # Get bounding rect with the new geometry
            x, y, self.warp_width, self.warp_height = cv2.boundingRect(np.array((
                dst_pt_ltop, dst_pt_rtop, dst_pt_lbottom, dst_pt_rbottom)))

            # Apply translation component to rotation matrix
            Mt = np.float32([[1., 0, -float(x)], [0, 1., -float(y)], [0, 0, 1.]])
            self._M = np.matmul(Mt, M)

            # Get image size coordinates in warped space
            dst_pt_ltop = get_projection_point_dst(
                coords_src=(0, 0), M=self._M)
            dst_pt_rtop = get_projection_point_dst(
        ...
        # Get image size coordinates in warped space
        dst_p1 = get_projection_point_dst(
            coords_src=self._Mpts["p1"], M=self._M)
        dst_p2 = get_projection_point_dst(
            coords_src=self._Mpts["p2"], M=self._M)
        dst_p3 = get_projection_point_dst(
            coords_src=self._Mpts["p3"], M=self._M)
        dst_p4 = get_projection_point_dst(
            coords_src=self._Mpts["p4"], M=self._M)
        self._Mdst_pts = {
            "p1":dst_p1, "p2":dst_p2, 
            "p3":dst_p3, "p4":dst_p4}
    ...
    def _rotate_space(self):
        ...
        self._Mpts["p1"], self._Mpts["p2"], self._Mpts["p3"], self._Mpts["p4"] = \
            self._Mpts["p4"], self._Mpts["p1"], self._Mpts["p2"], self._Mpts["p3"]
    ...
    def _move_pt(self, direction="up"):
        ...
        if len(self.pts):
            ...
            # if there's one
            key, _ = closest_neighbor(self._Mpts, 
                x=self.pts[self.pts_idx][0], 
                y=self.pts[self.pts_idx][1])
            if key is not None:
                self._Mpts[key] = (
                    self.pts[self.pts_idx][0], 
                    self.pts[self.pts_idx][1])
...
def main(argv):
    ...
    if intrinsic_file is not None:
        ...
        intrinsic = read_intrinsic_params(
            CONF_PATH=os.path.join(fwd, "configs"), 
