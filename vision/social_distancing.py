...
class SocialDistacing():
    ...
    def __init__(self, extrinsic, safe_tresh=2.0, intrinsic=None, 
        draw_warped=True):

        # NOTE: Intrinsic params no supported yet in extrinsic
        self.extrinsic = Extrinsic(
            ext_file_path=extrinsic,
            int_file_path=None)
        ...
        self.radar_img = create_radar(
            size=int(os.getenv(
                "SAFE_DISTANCING_RADAR_SIZE", default=300)),
            div=int(os.getenv(
                "SAFE_DISTANCING_RADAR_DIV", default=30)),
            color=(0, 255, 0),
            img=None)
    ...
    def analyse(self, img_src=None, detections=None):
        ...
        if detections is not None:
            ...
            if self.__src_img is not None:
                self.__src_detections = norm_predictions(
                    img_shape=self.__src_img.shape,
                    predictions=self.__src_detections)
        ...
        for idx, dst_detec in enumerate(self.__dst_detections):
            for idx2, aux_dst_detec in enumerate(self.__dst_detections):
                if dst_detec["idx"] != aux_dst_detec["idx"]:
                    ...
                    dx = (x1 - x2); dy = (y1 - y2)
                    dx = dx*self.extrinsic.ppmx if self.extrinsic.ppmx is not None else dx
                    dy = dy*self.extrinsic.ppmy if self.extrinsic.ppmy is not None else dy
        ...
        if img_src is not None:
            # get warped image
            self.__dst_img = self.extrinsic.get_warp_img(
                img_src=img_src)
    ...
    def draw_analysis(self):
        ...
        dst_img = image_resize(
            image=self.__dst_img.copy(), 
            height=radar_img.shape[0])
    ...
    def draw_detection(self, img, detection, closest=True, radius=3, 
        draw_neighbors=True):
        ...
        if draw_neighbors:
            for idx in detection["neighbors"]:
                ng_x = int(self.__dst_detections[idx]["box_base_norm"][0]*img.shape[1])
                ng_y = int(self.__dst_detections[idx]["box_base_norm"][1]*img.shape[0])
                cv2.line(img=img, pt1=pred_pt, pt2=(ng_x, ng_y), 
        ...
        if not detection["safe"]:
            cv2.circle(
                img=img, 
                center=pred_pt,  
                radius=radius*2, color=(0, 255, 255), thickness=1)
