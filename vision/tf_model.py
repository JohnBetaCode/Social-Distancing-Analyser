...
def parse_detections(predictions, min_score=0.5, max_predictions=20, 
    image_shape=None):
...
class TFModel(object):
    ...
    def __init__(self, saved_model_path, threshold=0.5, normalized=True):
        ...
        if tf.__version__ >= "2":
            self.imported = tf.saved_model.load(saved_model_path)
            self.model = self.imported.signatures["serving_default"]
        else:
    ...
    def predict(self, frame):
        ...
        frame = frame[None, ...]
        if tf.__version__ >= "2":
            outputs = self.model(inputs=tf.constant(frame))
            for key, val in outputs.items():
                outputs[key] = val.numpy()
        else:
            outputs = self.model(dict(inputs=frame))
        outputs = parse_detections(
            outputs, image_shape=image_size, min_score=self.threshold
        )
        return outputs
