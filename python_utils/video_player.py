...
class Player():
    ...
    def __init__(self, win_name, video_src_file, media_loop=True):
        ...
        self._WIN_MAX_WIDTH = int(
            os.getenv(key="WIN_WIDTH", default=640))
        self._WIN_MAX_HEIGHT = int(
            os.getenv(key="WIN_HEIGHT", default=480))
        ...
        self._win_menu = cv2.imread(os.path.join(
            os.path.dirname(os.path.abspath(__file__)), 
            "figures", "player_menu.jpg"))
        self._win_show_menu = True
        self._win_pause = True
        self._win_img = np.zeros((self._WIN_MAX_HEIGHT, 
            self._WIN_MAX_WIDTH, 3), np.uint8)
        ...
        # Capture attributes
        self.cap_img = np.zeros((self._WIN_MAX_HEIGHT, 
            self._WIN_MAX_WIDTH, 3), np.uint8)
    ...
    def _read_media_source(self, media_list):
        ...
        try:
            ...
            if os.path.isfile(file_path):
                ...
                if not len(self._src_list):
                    printlog(msg="No elements in {}".format(media_list), 
                        msg_type="ERROR")
                    exit()
            else:
        ...
        # Report any error saving de data
                ...
                printlog(msg="Error loading data list, {}".format(e), 
                    msg_type="ERROR")

        printlog(msg="Data list loaded from {}".format(media_list), 
            msg_type="OKGREEN")

        # Print video sources in list to be reproduced
        for data_src in self._src_list:
            print("\tfile:{}    \tExtrinsic:{}  \tIntrinsics".format(
                data_src["file_name"], 
                data_src["extrinsic"],
                data_src["intrinsic"]))
    ...
    def _start_capture(self, cap_idx=0):
        ...
        if not os.path.isfile(os.path.join(
            ...
            printlog(msg="file video {} no found".format(
    ...
    def _get_frame(self):
        ...
        if not self._cap_ret: 
            printlog(msg="Error getting video frame in {}".format(
    ...
    def _draw_gui(self, img):
        ...
        text_list = [
            "({}/{}) {}".format(
                self._cap_idx+1, len(self._src_list),
                self._src_list[self._cap_idx]["file_name"],
            ),
            "{:.2f}% - {}/{}".format(
                self._cap_frame/self._cap_frame_count*100,
                self._cap_frame_count, self._cap_frame,
            ),
            "size: {}x{}".format(
                self._cap_frame_width,
                self._cap_frame_height),
        ...
        print_text_list(img=img, tex_list=text_list, 
            color=(255, 0, 255), orig=(10, 25), fontScale=0.4)
    ...
    def snapshot(self):
        ...
        printlog(msg="capture saved as {} in capture folder".format(
            capture_name), msg_type="INFO")
    ...
    def reproduce(self, img_src=None, process_time=0):
        ...
        twait = (1./self._cap_rate - tock - process_time)*1000
        if twait <= 1:
            twait = 1
